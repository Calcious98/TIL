...
> 패키지: 여러 모듈을 체계적으로 모아서 꾸러미로 관리
- 패키지는 폴더 구조로 돼 있으며 각 폴더에는 `__init__.py`라는 파일이 있다 
  - `__init__.py`: 해당 폴더가 패키지의 일부인 것을 알려주는 역활 
  - 패키지를 초기화하는 코드를 넣을 수도 있고 아무 코드도 없는 빈 파일일 수도
### 패키지 사용하기
- import 패키지 내 모듈명: 패키지 모듈을 이용한다 
- 패키지명에서 시작해 모듈명까지 구분하기 위해 패키지명, 폴더명, 모듈명 사이에 . 을 입력 

- 패키지 폴더 안에 바로 모듈이 있다면 'import 패키지명 모들명 으로 모듈을 호출
- 패키지와 모듈 사이에 폴더가 있다면 'import 패키지명 폴더명 모둘명 으로 모듈을 호출

![](2022-08-12-10-19-22.png)
 + 패키지에서 모듈 내의 함수를 호출하는 법

- `from A import B` 형식을 이용하면 패키지 안에 있는 모듈 내 함수를 더 간단하게 호출
  - A: '패키지명.폴더명
  - B: 사용할 모듈명 

![](2022-08-12-10-21-19.png)

- imgread 모듈의 모든 함수를 바로 부르려면 `from 패키지명 폴더명
모듈명 import *` 를 이용

![](2022-08-12-10-22-15.png)
![](2022-08-12-10-23-08.png)

## 실습
---
### 패키지 만들기
![](2022-08-12-10-28-16.png)

![](2022-08-12-10-29-55.png)

### 패키지 사용하기 
1. `import 패키지 내 모듈명` 선언 

![](2022-08-12-10-44-19.png)

2. `from A import B`: 패키지 안에 있는 모듈 내 함수를 더 간단하게 호출 가능 

![](2022-08-12-10-45-23.png)

3. `*` :imgread 모듈의 모든 함수를 바로 부른다 

![](2022-08-12-10-47-09.png)

4. as 별명을 이용해 함수 호출 가능

![](2022-08-12-10-49-20.png)
...
try: 
  ...
  예외발행 여부와 무관하게 반드시 실행해야 할 명령
```
- try: 실행한 명령을 가지고 except or else로 보냄
- finally에는 보통 file close()을 넣는다 

![](2022-08-12-11-16-31.png)
![](2022-08-12-11-22-24.png)

## 오류 회피하기
- 프로그래밍을 하다 보면 특정 오류가 발생할 경우 그냥 통과시켜야 할 때

![](2022-08-12-11-18-20.png)

## 오류 일부러 발생시키기 
- raise 명령어를 사용해 오류를 강제로 발생시킨다. 
![](2022-08-12-11-19-20.png)